...
class TxBuilder {
  ...
  Future<List<String>> getChatList(String address) async {
    try {
      final accountResource = await client.getAccountResource(address, allContactsResourceType);
  ...
  Future<List<ChatMessage>> getMessages(String myAddress, String otherAddress) async {
    try {
      final accountResource = await client.getAccountResource(
          myAddress,
  ...
  Future<bool> checkChatEnabled(String address) async {
    try {
      final data = await client.getAccountResource(address, messageStoreResouceType);
